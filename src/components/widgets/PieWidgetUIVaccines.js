...
function __getDefaultLabel(data = []) {
  ...
  */
  return data.find((category) => {
    return category.name === 'Fully vaccinated';
  });
}
...
export function PieWidgetUIVaccines({
  name,
  data = [],
  formatter,
  tooltipFormatter,
  ...
  const updateLabel = (params) => {
    if (params.target) {
      ...
      const { option, serie } = getChartSerie(echart, params.target.seriesIndex);
      if (params.type == 'mouseover') {
        serie.data.forEach((category) => {
          category.label.show =
            category.name === serie.data[params.target.dataIndex].name;
          category.emphasis.label.show =
            category.name === serie.data[params.target.dataIndex].name;
        });
      } else {
        serie.data.forEach((category) => {
        ...
        //serie.data[0].emphasis.label.show = true;
        serie.data[0].label.show = true;
      }
  ...
  */
  ...
  useEffect(() => {
    if (chartInstance?.current) {
      ...
      zr.on('mouseover', mouseoverEvent);
...
PieWidgetUIVaccines.defaultProps = {
  name: null,
  formatter: (v) => v,
...
export default PieWidgetUIVaccines;
